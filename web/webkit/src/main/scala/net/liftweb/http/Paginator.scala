...
trait PaginatorSnippet[T] extends Paginator[T] {
  ...
  def paginate(xhtml: NodeSeq) = {
    bind(navPrefix, xhtml,
         TheBindParam("first", pageXml(0, firstXml)),
         TheBindParam("prev", pageXml(first-itemsPerPage max 0, prevXml)),
         FuncBindParam("allpages", {(n:NodeSeq) => pagesXml(0 until numPages, n)}),
         FuncBindParam("zoomedpages", {(ns: NodeSeq) => pagesXml(zoomedPages, ns)}),
         TheBindParam("next", pageXml(first+itemsPerPage min itemsPerPage*(numPages-1) max 0, nextXml)),
         TheBindParam("last", pageXml(itemsPerPage*(numPages-1), lastXml)),
         TheBindParam("records", currentXml),
         TheBindParam("recordsFrom", Text(recordsFrom)),
         TheBindParam("recordsTo", Text(recordsTo)),
         TheBindParam("recordsCount", Text(count.toString))
       )
  }
}
